...
class MyVxlanGpeNshIpClient(MyNshBaseClass):
    ...
    def datagram_received(self, data, addr):
        logger.info('gmy=>datagram_received')
        signal.alarm(0)
        logger.info("Received packet from SFF: %s", addr)
        logger.info("Packet dump: %s", binascii.hexlify(data))
...
def main(argv):
    ...
    local_port = 20020
    remote_sff_ip = "192.168.1.97"
    local_ip = "192.168.1.97"
    sfp_id = 0x000001
    sfp_index = 3
    trace_req = False
    num_trace_hops = 254
    inner_src_ip = "192.168.1.97"
    inner_dest_ip = "192.168.1.97"
    inner_src_port =20010
    inner_dest_port = 20020
    encapsulate = 'gpe-nsh-ipv4'  # make vxlan-gpe encapsulation default
    ctx1 = ctx2 = ctx3 = ctx4 = 0
    inner_src_eth_list =  [0x00,0x0c,0x29,0xd1,0x9e,0xa3]
    # [0x9c,0xc9, 0xb9, 0x27,0x00, 0x08]
    inner_dest_eth_list = [0x00,0x0c,0x29,0xd1,0x9e,0xa3]
