...
def find_sff_locator_by_ip(addr):
    ...
    """
    # logger.info('gmy=>find_sff_locator_by_ip')
    sff_name_list = []
    local_sff_topo = sfc_globals.get_sff_topo()
    # logger.info('gmy=>target'+addr)
    for sff_name, sff_value in local_sff_topo.items():
        try:
            for locator_value in sff_value['sff-data-plane-locator']:
                # logger.info('gmy=>'+locator_value['data-plane-locator']['ip'])
                # logger.info(type(addr))
                # logger.info(type(locator_value['data-plane-locator']['ip']))
                # logger.info( locator_value['data-plane-locator']['ip'] == addr)
...
def get_sff_from_odl(odl_ip_port, sff_name):
    ...
    if r.ok:
        ...
        logger.warning("=>Failed to GET SFF {} from ODL \发n".format(sff_name))
        return -1
...
def auto_sff_name():
    ...
    """
    sff_name = []
    logger.info("gmy=>auto_sff_name()" )
    for intf in netifaces.interfaces():
        addr_list_dict = netifaces.ifaddresses(intf)
        # Some interfaces have no address
        if addr_list_dict:
            logger.info("gmy=>addr_list_dict"+str(addr_list_dict))
            # Some interfaces have no IPv4 address.
            if netifaces.AF_INET in addr_list_dict:
                logger.info('gmy=>netifaces.AF_INET in addr_list_dict:')
                inet_addr_list = addr_list_dict[netifaces.AF_INET]

                for value in inet_addr_list:
                    logger.info('gmy=>addr %s', value['addr'])
                    # We return the list of all SFFs that match
                    sff_name = find_sff_locator_by_ip(value['addr'])
                    logger.info('gmy=>sff_name'+str(sff_name))

                    if sff_name:
                        sfc_globals.set_my_sff_name(sff_name)
                        # sff_name = sfc_globals.get_my_sff_name()

                        logger.info("gmy=>!!!!!Auto SFF name is: %s", sff_name)
                        return 0
            if netifaces.AF_INET6 in addr_list_dict:
                inet_addr_list = addr_list_dict[netifaces.AF_INET6]
                logger.info('netifaces.AF_INET6 in addr_list_dict')
                for value in inet_addr_list:
