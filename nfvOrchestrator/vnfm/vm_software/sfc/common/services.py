...
class BasicService(object):
    def __init__(self, loop):
        ...
        """
        logger.info('gmy=>BasicService __init__')
        self.loop = loop
        self.transport = None
    ...
    def _decode_headers(self, data):
        ...
        Decode the incoming packet for info purposes and to strip out various
        header values.

        """
        logger.info('gmy=>BasicService _decode_headers')
    ...
    def _process_incoming_packet(self, data, addr):
        ...
        """
        logger.info('gmy=>BasicService _process_incoming_packet')
    ...
    def _update_metadata(self, data,
        ...
        """
        logger.info('gmy=>BasicService _update_metadata')
    ...
    def connection_made(self, transport):
        logger.info('gmy=>BasicService connection_made')
    ...
    def datagram_received(self, data, addr):
        ...
        """
        logger.info('gmy=>BasicService datagram_received')
        logger.info('%s service received packet from %s:', self.service_type, addr)
        logger.info('%s %s', addr, binascii.hexlify(data))
        packet = (data, addr)
        try:
    ...
    def process_datagram(self, data, addr):
        ...
        """
        logger.info('gmy=>BasicService process_datagram')
        logger.info('gmy=>process_datagram')
        logger.info('%s service received packet from %s:', self.service_type, addr)
        logger.info('%s %s', addr, binascii.hexlify(data))
        ...
        if nsh_decode.is_data_message(data):
            ...
            if nsh_decode.is_vxlan_nsh_legacy_message(data):
                ...
                # gmy!!! lower does open ovs ,skip these code
                logger.info('gmy=>is_vxlan_nsh_legacy_message')
                addr_l = list(addr)
                addr_l[1] = 6633
                addr = tuple(addr_l)

            self.transport.sendto(rw_data, addr)
            logger.info('Sending packets over')
    ...
    def read_queue(self):
        ...
        """
        logger.info('gmy=>BasicService read_queue')
        logger.info('gmy=>read_queue')
        try:
            while True:
                logger.info('gmy=>read_queue  xunhuan')
                packet = self.packet_queue.get(block=True)
                logger.info('gmy=>read_queue  getpacket')
                self.process_datagram(data=packet[0], addr=packet[1])
                logger.info('gmy=>read_queue  process_datagram over')
                self.packet_queue.task_done()
                logger.info('gmy=>read_queue  task_done over')
        except:
    ...
    def process_trace_pkt(self, rw_data, data):
        logger.info('gmy=>BasicService process_trace_pkt')
    ...
    def connection_refused(exc):
        logger.info('gmy=>BasicService connection_refused')
        logger.error('Connection refused: %s', exc)

    def connection_lost(self, exc):
        logger.info('gmy=>BasicService connection_lost')
...
class ControlUdpServer(BasicService):
    def __init__(self, loop):
        ...
        """
        logger.info('gmy=>ControlUdpServer __init__')
    ...
    def datagram_received(self, data, addr):
        logger.info('gmy=>ControlUdpServer datagram_received')
...
class MySffServer(BasicService):
    ...
    def _lookup_next_sf(self, service_path, service_index):
        ...
        """
        logger.info('gmy=>MySffServer _lookup_next_sf')
    ...
    def _get_packet_bearing(packet):
        ...
        """
        logger.info('gmy=>MySffServer _get_packet_bearing')
        ip_header = packet[:20]
    ...
    def _process_incoming_packet(self, data, addr):
        ...
        """
        logger.info('gmy=>MySffServer _process_incoming_packet')
        logger.info("gmy=>%s: sff Processing packet from: %s", self.service_type, addr)
        address = ()
        rw_data = bytearray(data)
        self._decode_headers(data)
        ...
        if nsh_decode.is_data_message(data):
            ...
            if next_hop != SERVICE_HOP_INVALID:
                ...
                logger.info("%s: Sending packets to  next hop: %s", self.service_type, address)
            ...
            # send packet to its original destination
                ...
                logger.info("%s: Packet dump: %s", self.service_type, binascii.hexlify(rw_data))
                ...
                if inner_packet:
                    ...
                    sock_raw.sendto(inner_packet, (bearing['d_addr'],
                                                   bearing['d_port']))
                    logger.info(sock_raw)
                    logger.info(inner_packet)
    ...
    def process_datagram(self, data, addr):
        ...
        """
        logger.info('gmy=>MySffServer process_datagram')
        logger.info('%s: Received a packet from: %s', self.service_type, addr)

        self._process_incoming_packet(data, addr)

    def connection_lost(self, exc):
        logger.info('gmy=>MySffServer connection_lost')
        logger.error('stop', exc)

    @staticmethod
    def error_received(exc):
        logger.info('gmy=>MySffServer error_received')
