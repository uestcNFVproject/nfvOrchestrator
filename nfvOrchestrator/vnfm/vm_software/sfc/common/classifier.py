...
logger.warn(sys.platform)
logger.warn('')
logger.warn(sys.platform.startswith('linux'))
...
class NfqClassifier(metaclass=Singleton):
    def __init__(self):
        ...
        """
        logger.info('gmy=>classifier __init__')
        # internal data-store
        self.rsp_2_sff = {}
    ...
    def process_packet(self, packet):
        ...
        :type packet: `:class:netfilterqueue.Packet`
        """
        logger.info('gmy=>process_packet')
        try:
    ...
    def packet_collector(self):
        ...
        """
        logger.info('gmy=>packet_collector NFQ_AVAILABLE')
        logger.info(NFQ_AVAILABLE)
        if not NFQ_AVAILABLE:
            logger.error('Classifier can\'t start\n\n'
                         '*** NetfilterQueue not supported or installed ***\n')
            return
    ...
    def parse_ace(self, ace_matches):
        ...
        logger.info('Parsing ACE...ace_matches:')
        logger.info(ace_matches)
        ...
        for port_range in ports:
            logger.info('outer: port_range')
            logger.info(port_range)
    ...
    def process_acl(self, acl_data):
        ...
        """
        logger.info('gmy=>process_acl')
        logger.info(acl_data)
        for acl in acl_data['acl']:
            ...
            for ace in acl['access-list-entries']['ace']:
                ...
                logger.info('gmy=>ace_rule_cmd start')
                ace_rule_cmd = self.parse_ace(ace['matches'])
                logger.info('gmy=>ace_rule_cmd')
                logger.info(ace_rule_cmd)
